...
public sealed class RemoveListCommandHandler(
    IUnitOfWork unitOfWork,
    IUser currentUser,
    ProjectAccessService accessService)
    : IRequestHandler<RemoveListCommand, Result<Deleted>>
{
    public async Task<Result<Deleted>> Handle(RemoveListCommand command, CancellationToken ct)
    {
        ...
        var result = board.RemoveList(command.ListId);
        if (result.IsFailure)
