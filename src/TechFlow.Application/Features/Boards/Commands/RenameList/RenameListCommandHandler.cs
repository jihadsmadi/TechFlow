...
public sealed class RenameListCommandHandler(
    ...
    : IRequestHandler<RenameListCommand, Result<Updated>>
{
    public async Task<Result<Updated>> Handle(RenameListCommand command, CancellationToken ct)
    {
        ...
        var result = board.RenameList(command.ListId, command.Name);
        if (result.IsFailure)
