...
public sealed class ReorderListsCommandHandler(
    IUnitOfWork unitOfWork,
    IUser currentUser,
    ProjectAccessService accessService)
    : IRequestHandler<ReorderListsCommand, Result<Updated>>
{
    public async Task<Result<Updated>> Handle(ReorderListsCommand command, CancellationToken ct)
    {
        ...
        var result = board.ReorderLists(command.OrderedListIds);
        if (result.IsFailure)
