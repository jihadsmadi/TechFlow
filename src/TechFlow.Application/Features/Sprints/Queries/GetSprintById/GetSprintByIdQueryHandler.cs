...
public sealed class GetSprintByIdQueryHandler(
    ...
    : IRequestHandler<GetSprintByIdQuery, Result<SprintDto>>
{
    public async Task<Result<SprintDto>> Handle(GetSprintByIdQuery query, CancellationToken ct)
    {
        ...
        var sprint = await unitOfWork.Sprints.GetByIdWithItemsAsync(query.SprintId, ct);
        if (sprint is null)
